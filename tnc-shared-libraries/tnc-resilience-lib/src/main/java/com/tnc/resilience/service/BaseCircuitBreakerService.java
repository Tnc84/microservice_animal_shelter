...
public abstract class BaseCircuitBreakerService<T, ID> {
    ...
    public Optional<T> findById(ID id) {
        ...
        Supplier<Optional<T>> decoratedSupplier = CircuitBreaker
    ...
    public void deleteById(ID id) {
        ...
        Runnable decoratedRunnable = CircuitBreaker
                .decorateRunnable(circuitBreaker, runnable);
     ...
     */
    public <R> R executeWithResilience(Supplier<R> operation) {
        CircuitBreaker circuitBreaker = circuitBreakerRegistry.circuitBreaker("database");
        Retry retry = retryRegistry.retry("database");
        TimeLimiter timeLimiter = timeLimiterRegistry.timeLimiter("database");

        Supplier<R> decoratedSupplier = CircuitBreaker
     ...
     */
    public <R> CompletableFuture<R> executeAsyncWithResilience(Supplier<CompletableFuture<R>> operation) {
        CircuitBreaker circuitBreaker = circuitBreakerRegistry.circuitBreaker("database");
        Retry retry = retryRegistry.retry("database");
        TimeLimiter timeLimiter = timeLimiterRegistry.timeLimiter("database");

        Supplier<CompletableFuture<R>> decoratedSupplier = CircuitBreaker
                .decorateSupplier(circuitBreaker, operation);
        decoratedSupplier = Retry.decorateSupplier(retry, decoratedSupplier);
